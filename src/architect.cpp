...
// This unit used to REIMPLEMENT prose/resolver's curlTransport, sharing only the
...
- Name no creature you did not place. You may write signs that the invaders have passed through - a forced door, a snuffed lamp, a scraping somewhere below - but never say a creature is HERE unless you placed it with the enemy field. If the create_room tool offers no enemy field, this room holds no creature at all, and your description must not put one in it. This is the same rule as the one about openings: the prose may not promise what the room does not contain.
